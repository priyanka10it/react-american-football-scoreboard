...
import "./App.css";
 ...
 function App() {
  ...
  const[ishomeScore, sethomeScore] = useState(0);
  const[isawayScore, setawayScore] = useState(0);

  const homefieldGoal = e =>{
    sethomeScore(ishomeScore + 3);
  }
  const hometouchDown = e =>{
    sethomeScore(ishomeScore + 7);
  }
  const awayfieldGoal = e =>{
    setawayScore(isawayScore + 3);
  }
  const awaytouchDown = e =>{
    setawayScore(isawayScore + 7);
  }
  return (
      ...
      <section className="scoreboard">
        <div className="topRow">
          <div className="home">
            ...
            <div className="home__score">{ishomeScore}</div>
          </div>
          <div className="timer">00:03</div>
          <div className="away">
            <h2 className="away__name">Tigers</h2>
             <div className="away__score">{isawayScore}</div>
          </div>
        </div>
        <BottomRow />
      </section>
